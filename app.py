...
def render_price_context(intraday: pd.DataFrame) -> Tuple[float, float]:
    ...
    cols = st.columns(2)
    spy = latest_price(intraday, DEFAULT_SPOT_SYMBOL)
    vix = latest_price(intraday, DEFAULT_VOL_SYMBOL)
    with cols[0]:
        st.metric("SPY last", f"{spy:.2f}" if not math.isnan(spy) else "n/a")
    with cols[1]:
        st.metric("VIX last", f"{vix:.2f}" if not math.isnan(vix) else "n/a")
    return spy, vix
